...
namespace plugin\jicang\app\service;

use plugin\jicang\app\model\scopes\AccessDataScope;
use plugin\jicang\app\model\system\UserModel;
use plugin\jicang\app\service\monitor\LogininforService;
use plugin\jicang\app\service\system\MenuService;
use plugin\jicang\app\service\system\RoleMenuService;
...
class AuthService
{
    ...
    public function setLoginUser($user)
    {
        ...
        if (!empty($user['uuid'])) {
            $uuid = $user['uuid'];
            $user['expire_time'] = date("Y-m-d H:i:s", time() + config('plugin.jicang.token.jwt.token_expire'));
            $cacheKey = (new Token())->getTokenKey($uuid);
            Util::getRedis()->setex($cacheKey, config('plugin.jicang.token.jwt.token_expire'), serialize($user));
        }
    }
